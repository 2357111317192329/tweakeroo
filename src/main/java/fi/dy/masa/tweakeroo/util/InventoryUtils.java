...
public class InventoryUtils
{
    ...
    public static void trySwapElytraIfNearlyBroken(PlayerEntity player)
    {
        // 檢查是否穿戴了鞘翅
        ItemStack chestArmor = player.getEquippedStack(EquipmentSlot.CHEST);
        
        if (!chestArmor.isEmpty() && chestArmor.getItem() == Items.ELYTRA)
        {
            // 使用與工具相同的耐久閾值邏輯
            int minDurability = getMinDurability(chestArmor);
            //System.out.println(minDurability);

            if (isItemAtLowDurability(chestArmor, minDurability))
            {
                // 只替換耐久度更高的鞘翅
                swapItemWithHigherDurabilityToEquipmentSlot(
                    player, 
                    EquipmentSlot.CHEST, 
                    chestArmor, 
                    minDurability + 1
...
//        Tweakeroo.LOGGER.error("isBetterTool(): test [{}], prev [{}], state [{}] // isTool [{}] // isMisc [{}]", testedStack.toString(), previousTool.toString(), state.toString(), isTool, isMisc);
    ...
    private static void swapItemWithHigherDurabilityToEquipmentSlot(PlayerEntity player, EquipmentSlot slotType, ItemStack stackReference, int minDurabilityLeft)
    {
        ScreenHandler container = player.playerScreenHandler;
        
        // 第一步：尋找耐久度更高的替代品
        int slotWithItem = findSlotWithSuitableReplacementToolWithDurabilityLeft(
                container, stackReference, minDurabilityLeft);
        
        if (slotWithItem != -1)
        {
            swapItemToEquipmentSlot(player, slotType, slotWithItem);
            InfoUtils.printActionbarMessage("tweakeroo.message.swapped_low_durability_item_for_better_durability");
            return;
        }
    }
